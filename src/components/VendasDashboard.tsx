...
import { ProductSaleRow, formatK } from '@/lib/sheets';
...
// Types para Chart.js (apenas presença)
declare global {
  interface Window {
    Chart?: unknown;
  }
...
// Tipos foram extraídos para '@/features/sales/types'
...
export default function VendasDashboard() {
  ...
  useEffect(() => {
    ...
    setChartData(filters.chartData);
  ...
  const renderCharts = () => {
    ...
    const ChartCtor = (window as unknown as { Chart: new (ctx: unknown, config: unknown) => unknown }).Chart;
    ...
    if (ctxClientes) {
      ...
      newCharts.clientes = new ChartCtor(ctxClientes, {
        type: 'doughnut',
        data: {
    ...
    if (ctxProdutos && chartData.topProdutos) {
      ...
      newCharts.produtos = new ChartCtor(ctxProdutos, {
        type: 'doughnut',
        data: {
    ...
    if (meta?.hasCustomerType) {
      ...
      if (ctxTiposCliente && chartData.topTiposCliente) {
        ...
        newCharts.tiposCliente = new ChartCtor(ctxTiposCliente, {
          type: 'doughnut',
          data: {
    ...
    if (ctxEng) {
      newCharts.engajamento = new ChartCtor(ctxEng, {
        type: 'bar',
        data: {
      ...
      <main className={vendasStyles.container}>
        ...
        <section className={vendasStyles['table-wrap']} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <ProductsByClientTable
            ...
            setClientPickerQuery={setClientPickerQuery}
            clientPickerTemp={clientPickerTemp}
            setClientPickerTemp={setClientPickerTemp}
          ...
          <ClientsByProductTable
            ...
            setSelectProduto={setSelectProduto}
            ...
            productPickerTemp={productPickerTemp}
            setProductPickerTemp={setProductPickerTemp}
            setModalTitle={setModalTitle}
            setModalData={setModalData}
            setShowModal={setShowModal}
          />
